...
export default function HomePageContent() {
    ...
    if (role === 'admin') {
        ...
        callToAction = "Mergeți la Dashboard pentru a gestiona utilizatorii și evenimentele.";
        colorClass = "text-theme-primary-dark";
        icon = "👑";
    } else if (role === 'lider') {
    ...
    return (
        ...
        <div className="p-6 sm:p-10 bg-white shadow-2xl rounded-2xl max-w-xl w-full mx-auto text-center border-t-8 border-theme-primary/80 transform hover:shadow-3xl transition duration-300">
            <div className={`text-5xl sm:text-7xl mb-4 ${colorClass} animate-bounce-slow`}>{icon}</div>
            
            <h1 className={`text-3xl sm:text-4xl font-extrabold mb-2 ${colorClass} leading-tight text-gray-900`}>
                {welcomeMessage}
            </h1>
            ...
            <p className="text-base sm:text-lg mb-8 max-w-sm mx-auto">
                {callToAction}
            </p>
            ...
            <div className="flex flex-col space-y-3 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
                ...
                {/* Buton principal (Dashboard sau Login/Resurse) */}
                {
                    (role === 'admin' || role === 'lider') && 
                        <>
                            <Link
                                href="/dashboard"
                                className="w-full sm:w-auto inline-block bg-theme-primary hover:bg-theme-primary-dark text-white font-bold py-3 px-6 rounded-xl transition duration-200 shadow-lg transform hover:scale-[1.02]"
                            >
                ...
                <Link
                    href="/events"
                    className="w-full sm:w-auto inline-block bg-gray-700 hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl transition duration-200 shadow-md transform hover:scale-[1.02]"
                >
                    Vezi Calendarul
                </Link>
