    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 relative">
                ...
                <h2 className="text-2xl font-bold text-theme-primary-dark mb-2">{event.title}</h2>
                ...
                {currentUser && !currentUser.isAnonymous && (
                    <div className="mt-6">
                        {isCurrentUserParticipating ? (
                            <button onClick={handleCancelRsvp} disabled={isSubmitting} className="w-full bg-theme-danger hover:bg-opacity-90 text-white font-bold py-2 px-4 rounded transition disabled:opacity-50">
                                {isSubmitting ? 'Se anulează...' : 'Renunță la participare'}
                            </button>
                        ) : (
                            <button onClick={handleRsvp} disabled={isSubmitting} className="w-full bg-theme-success hover:bg-opacity-90 text-white font-bold py-2 px-4 rounded transition disabled:opacity-50">
                ...
                <div className="mt-6 border-t pt-4">
                    <h3 className="text-lg font-semibold mb-2 text-theme-primary-dark">Participanți ({participants.length})</h3>
